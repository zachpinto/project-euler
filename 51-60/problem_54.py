...
# Helper function to convert card value to numeric value
def card_value(card):
    value_str = '23456789TJQKA'
    return value_str.index(card[0])

# Helper function to determine the rank of a hand
def hand_rank(hand):
    values = sorted([card_value(card) for card in hand], reverse=True)
    suits = [card[1] for card in hand]
    value_counts = Counter(values).most_common()
    flush = len(set(suits)) == 1
    straight = len(set(values)) == 5 and values[0] - values[-1] == 4

    if flush and values == [12, 11, 10, 9, 8]:
        return (9,)  # Royal Flush
    if flush and straight:
        return (8, values[0])  # Straight Flush
    if value_counts[0][1] == 4:
        return (7, value_counts[0][0], value_counts[1][0])  # Four of a Kind
    if value_counts[0][1] == 3 and value_counts[1][1] == 2:
        return (6, value_counts[0][0], value_counts[1][0])  # Full House
    if flush:
        return (5, values)  # Flush
    if straight:
        return (4, values[0])  # Straight
    if value_counts[0][1] == 3:
        return (3, value_counts[0][0], values)  # Three of a Kind
    if value_counts[0][1] == 2 and value_counts[1][1] == 2:
        return (2, value_counts[0][0], value_counts[1][0], values)  # Two Pairs
    if value_counts[0][1] == 2:
        return (1, value_counts[0][0], values)  # One Pair
    return (0, values)  # High Card

# Function to determine the winner between two hands
def compare_hands(hand1, hand2):
    return hand_rank(hand1) > hand_rank(hand2)

def main():
    # Load the poker hands from text input
    hands_text = """
    ...
    hands = [line.strip() for line in hands_text.strip().split('\n') if line.strip()]
