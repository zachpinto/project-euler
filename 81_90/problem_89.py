...
def characters_saved(original, minimal):
    return len(original) - len(minimal)


def main():
    roman_list = [line.strip() for line in roman_numerals.strip().split('\n')]

    total_saved = 0
    for roman in roman_list:
        minimal = minimal_roman(roman)
        saved = characters_saved(roman, minimal)
        total_saved += saved
        print(f"Original: {roman}, Minimal: {minimal}, Saved: {saved}")
